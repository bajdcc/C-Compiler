...
#define	UI_TIMER_DRAGSEL			1001
#define	IS_NON_ASCII(tchar)			((tchar)<_T('\0')||(tchar)>_T('\xFF'))

HINSTANCE CUITextView::s_hResourceInst = NULL;

// CUITextView

IMPLEMENT_DYNCREATE(CUITextView, CView)

#pragma region MASSAGE_MAP
BEGIN_MESSAGE_MAP(CUITextView, CView)
	ON_WM_CREATE()
	ON_WM_DESTROY()
...
END_MESSAGE_MAP()
#pragma endregion MASSAGE_MAP

#define EXPAND_PRIMITIVE(impl, func)	\
	void CUITextView::On##func() { impl(FALSE); }	\
	void CUITextView::OnExt##func() { impl(TRUE); }
...
#ifdef _DEBUG
void CUITextView::AssertValid() const
{
	CView::AssertValid();
}

#ifndef _WIN32_WCE
void CUITextView::Dump(CDumpContext& dc) const
{
	CView::Dump(dc);
}
#endif
...
/////////////////////////////////////////////////////////////////////////////
// CUITextView construction/destruction
...
BOOL CUITextView::PreCreateWindow(CREATESTRUCT& cs)
{
...
/////////////////////////////////////////////////////////////////////////////
// CUITextView color index

COLORREF CUITextView::GetColor(int nColorIndex)
{
	switch (nColorIndex)
	{
		//		Base colors
	...
	case COLORINDEX_BKGND:
		return ::GetSysColor(COLOR_WINDOW);
	case COLORINDEX_LINE_BKGND:
		return ::GetSysColor(COLOR_WINDOW);
	case COLORINDEX_SEPA_LINE:
		return RGB(165, 165, 165);
	case COLORINDEX_SEPA_LINE_NO:
	...
	case COLORINDEX_KILLFOCUS_SELBKGND:
		return RGB(229, 235, 241);
...
void CUITextView::GetSelection(CPoint &ptStart, CPoint &ptEnd)
{
	PrepareSelBounds();
...
CUITextBuffer *CUITextView::LocateTextBuffer()
{
	return NULL;
}

int CUITextView::GetLineActualLength(int nLineIndex)
{
...
void CUITextView::ScrollToChar(int nNewOffsetChar, BOOL bNoSmoothScroll /*= FALSE*/, BOOL bTrackScrollBar /*= TRUE*/)
{
...
void CUITextView::ScrollToLine(int nNewTopLine, BOOL bNoSmoothScroll /*= FALSE*/, BOOL bTrackScrollBar /*= TRUE*/)
{
...
void CUITextView::ExpandChars(LPCTSTR pszChars, int nOffset, int nCount, CString &line)
{
	if (nCount <= 0)
	{
		line = _T("");
...
void CUITextView::DrawLineHelperImpl(CDC *pdc, CPoint &ptOrigin, const CRect &rcClip,
									 LPCTSTR pszChars, int nOffset, int nCount)
{
	...
	if (nCount > 0)
	{
		...
		int length = 0; TCHAR* pch = line.GetBuffer();
		while (*pch != _T('\0'))
		{
			if (IS_NON_ASCII(*pch)) length++; length++; pch++;
		}
		ptOrigin.x += GetCharWidth() * length /*line.GetLength()*/;
	}
}

void CUITextView::DrawLineHelper(CDC *pdc, CPoint &ptOrigin, const CRect &rcClip, int nColorIndex,
									 LPCTSTR pszChars, int nOffset, int nCount, CPoint ptTextPos)
{
	if (nCount > 0)
	{
		...
		{
			COLORREF crOldBk = pdc->SetBkColor(m_bFocused ? GetColor(COLORINDEX_SELBKGND) : GetColor(COLORINDEX_KILLFOCUS_SELBKGND));
...
void CUITextView::GetLineColors(int nLineIndex, COLORREF &crBkgnd,
...
DWORD CUITextView::GetParseCookie(int nLineIndex)
{
	int nLineCount = GetLineCount();
	if (m_pdwParseCookies == NULL)
	{
...
void CUITextView::DrawSingleLine(CDC *pdc, const CRect &rc, int nLineIndex)
{
	...
	if (frect.right > frect.left)
	{
		...
		{
			pdc->FillSolidRect(frect.left, frect.top, GetCharWidth(), frect.Height(), GetColor(COLORINDEX_BKGND));
...
DWORD CUITextView::GetLineFlags(int nLineIndex)
{
...
void CUITextView::DrawMargin(CDC *pdc, const CRect &rect, int nLineIndex)
{
	if (! m_bSelMargin)
	{
	...
	*/
	...
	rt.right = GetMarginMarkWidth();
	pdc->FillSolidRect(rt, GetColor(COLORINDEX_SELMARGIN));			// Bookmark Margin

	rt.left = rt.right; rt.right = rect.right;
	pdc->FillSolidRect(rt, GetColor(COLORINDEX_BKGND));				// Line Margin

	if (nLineIndex >= 0) // Valid
	{
		if (nLineIndex > 10000)
			nLineIndex %= 10000;
		CRect rtNo = rt; rtNo.OffsetRect(2, 0);
		CString lineNo; lineNo.Format(_T("%4d"), nLineIndex + 1);
		COLORREF oldColor = pdc->SetTextColor(GetColor(COLORINDEX_SEPA_LINE_NO));
		CFont* oldFont = pdc->SelectObject(GetFont());
		pdc->DrawText(lineNo, &rtNo, DT_LEFT | DT_VCENTER);			// Line No
		pdc->SetTextColor(oldColor);
		pdc->SelectObject(oldFont);
	}

	rt.right -= 5; rt.InflateRect(0, 1);
	CPen cPen(PS_SOLID, 1, GetColor(COLORINDEX_SEPA_LINE));			// Line Separater
	CPen* pOldPen = pdc->SelectObject(&cPen);
	pdc->MoveTo(CPoint(rt.right, rt.top)); pdc->LineTo(CPoint(rt.right, rt.bottom));
...
BOOL CUITextView::IsInsideSelBlock(CPoint ptTextPos)
{
...
BOOL CUITextView::IsInsideSelection(const CPoint &ptTextPos)
{
	PrepareSelBounds();
	return IsInsideSelBlock(ptTextPos);
}

void CUITextView::PrepareSelBounds()
{
...
void CUITextView::OnDraw(CDC *pdc)
{
	CRect rcClient;
...
void CUITextView::UpdateCaret()
{
	...
	{
		CreateSolidCaret(1, GetLineHeight());
...
int CUITextView::GetTabSize()
{
	ASSERT(m_nTabSize >= 0 && m_nTabSize <= 64);
	return m_nTabSize;
}

void CUITextView::SetTabSize(int nTabSize)
{
...
CFont *CUITextView::GetFont(BOOL bItalic /*= FALSE*/, BOOL bBold /*= FALSE*/)
{
	int nIndex = 0;
	if (bBold)
		nIndex |= 1;
...
void CUITextView::CalcLineCharDim()
{
	CDC *pdc = GetDC();
...
int CUITextView::GetMaxLineLength()
{
...
CUITextView *CUITextView::GetSiblingView(int nRow, int nCol)
{
	...
	if (pWnd == NULL || ! pWnd->IsKindOf(RUNTIME_CLASS(CUITextView)))
		return NULL;
	return (CUITextView *) pWnd;
}

void CUITextView::OnInitialUpdate()
{
	...
	if (pSplitter != NULL)
	{
		...
		if (nRow > 0)
		{
			CUITextView *pSiblingView = GetSiblingView(0, nCol);
		...
		if (nCol > 0)
		{
			CUITextView *pSiblingView = GetSiblingView(nRow, 0);
			if (pSiblingView != NULL && pSiblingView != this)
			{
...
/////////////////////////////////////////////////////////////////////////////
// CUITextView printing

void CUITextView::OnPrepareDC(CDC* pDC, CPrintInfo* pInfo) 
{
...
BOOL CUITextView::OnPreparePrinting(CPrintInfo* pInfo)
{
	return DoPreparePrinting(pInfo);
}

int CUITextView::PrintLineHeight(CDC *pdc, int nLine)
{
...
void CUITextView::GetPrintHeaderText(int nPageNum, CString &text)
{
	ASSERT(m_bPrintHeader);
	text = _T("");
}

void CUITextView::GetPrintFooterText(int nPageNum, CString &text)
{
	ASSERT(m_bPrintFooter);
	text.Format(_T("Page %d/%d"), nPageNum, m_nPrintPages);
}

void CUITextView::PrintHeader(CDC *pdc, int nPageNum)
{
...
void CUITextView::PrintFooter(CDC *pdc, int nPageNum)
{
...
void CUITextView::RecalcPageLayouts(CDC *pdc, CPrintInfo *pInfo)
{
...
void CUITextView::OnBeginPrinting(CDC *pdc, CPrintInfo *pInfo)
{
...
void CUITextView::OnEndPrinting(CDC *pdc, CPrintInfo *pInfo)
{
...
void CUITextView::OnPrint(CDC* pdc, CPrintInfo* pInfo) 
{
	if (m_pnPages == NULL)
	{
...
/////////////////////////////////////////////////////////////////////////////
// CUITextView message handlers

int CUITextView::GetLineCount()
{
	if (m_pTextBuffer == NULL)
...
int CUITextView::GetLineLength(int nLineIndex)
{
	if (m_pTextBuffer == NULL)
		return 0;
...
LPCTSTR CUITextView::GetLineChars(int nLineIndex)
{
	if (m_pTextBuffer == NULL)
		return NULL;
...
void CUITextView::AttachToBuffer(CUITextBuffer *pBuf /*= NULL*/)
{
	if (m_pTextBuffer != NULL)
...
void CUITextView::DetachFromBuffer()
{
	if (m_pTextBuffer != NULL)
	{
		m_pTextBuffer->RemoveView(this);
...
int CUITextView::GetScreenLines()
{
	if (m_nScreenLines == -1)
	{
		CRect rect;
...
BOOL CUITextView::GetItalic(int nColorIndex)
{
	return FALSE;
}

BOOL CUITextView::GetBold(int nColorIndex)
{
	return FALSE;
}

int CUITextView::GetScreenChars()
{
	if (m_nScreenChars == -1)
	{
		CRect rect;
...
BOOL CUITextView::OnEraseBkgnd(CDC *pdc) 
{
	return TRUE;
}

void CUITextView::OnSize(UINT nType, int cx, int cy) 
{
...
void CUITextView::UpdateSiblingScrollPos(BOOL bHorz)
{
	...
	if (pSplitterWnd != NULL)
	{
		...
		if (bHorz)
		{
			...
			for (int nCol = 0; nCol < nCols; nCol ++)
			{
				...
				{
					CUITextView *pSiblingView = GetSiblingView(nCurrentRow, nCol);
		...
		else
		{
			...
			for (int nRow = 0; nRow < nRows; nRow ++)
			{
				...
				{
					CUITextView *pSiblingView = GetSiblingView(nRow, nCurrentCol);
					if (pSiblingView != NULL)
...
void CUITextView::OnUpdateSibling(CUITextView *pUpdateSource, BOOL bHorz)
{
	if (pUpdateSource != this)
	{
		ASSERT(pUpdateSource != NULL);
		ASSERT_KINDOF(CUITextView, pUpdateSource);
		if (bHorz)
		{
...
void CUITextView::RecalcVertScrollBar(BOOL bPositionOnly /*= FALSE*/)
{
	SCROLLINFO si;
	si.cbSize = sizeof(si);
	if (bPositionOnly)
...
void CUITextView::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
	...
	// lock scroll
	if (nLineCount > nPageLines)
		nLineCount -= nPageLines;
	else
		nLineCount = 0;
...
void CUITextView::RecalcHorzScrollBar(BOOL bPositionOnly /*= FALSE*/)
{
...
void CUITextView::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
...
BOOL CUITextView::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
...
CPoint CUITextView::ClientToText(const CPoint &point)
{
	int nLineCount = GetLineCount();
	...
	while (nIndex < nLength)
	{
		...
		else if (IS_NON_ASCII(pszLine[nIndex]))
			nCurPos += 2;
		else
			nCurPos ++;
...
#ifdef _DEBUG
void CUITextView::AssertValidTextPos(const CPoint &point)
{
	if (GetLineCount() > 0)
	{
...
CPoint CUITextView::TextToClient(const CPoint &point)
{
...
void CUITextView::InvalidateLines(int nLine1, int nLine2, BOOL bInvalidateMargin /*= FALSE*/)
{
...
void CUITextView::SetSelection(const CPoint &ptStart, const CPoint &ptEnd)
{
...
void CUITextView::AdjustTextPoint(CPoint &point)
{
	point.x += GetCharWidth() / 2;	//todo
}

void CUITextView::OnSetFocus(CWnd* pOldWnd) 
{
...
DWORD CUITextView::ParseLine(DWORD dwCookie, int nLineIndex, TEXTBLOCK *pBuf, int &nActualItems)
{
	return 0;
}

int CUITextView::CalculateActualOffset(int nLineIndex, int nCharIndex)
{
	int nLength = GetLineLength(nLineIndex);
...
int CUITextView::ApproxActualOffset(int nLineIndex, int nOffset)
{
	if (nOffset == 0)
		return 0;
...
void CUITextView::EnsureVisible(CPoint pt)
{
	//	Scroll vertically
	int nLineCount = GetLineCount();
...
void CUITextView::OnKillFocus(CWnd* pNewWnd) 
{
...
void CUITextView::OnSysColorChange() 
{
	CView::OnSysColorChange();
	Invalidate();
}

void CUITextView::GetText(const CPoint &ptStart, const CPoint &ptEnd, CString &text)
{
	if (m_pTextBuffer != NULL)
...
void CUITextView::UpdateView(CUITextView *pSource, IUpdateContext *pContext,
...
HINSTANCE CUITextView::GetResourceHandle()
{
...
int CUITextView::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
...
void CUITextView::SetAnchor(const CPoint &ptNewAnchor)
{
	ASSERT_VALIDTEXTPOS(ptNewAnchor);
	m_ptAnchor = ptNewAnchor;
}

void CUITextView::OnEditOperation(int nAction, LPCTSTR pszText)
{
}

BOOL CUITextView::PreTranslateMessage(MSG *pMsg) 
{
...
CPoint CUITextView::GetCursorPos()
{
	return m_ptCursorPos;
}

void CUITextView::SetCursorPos(const CPoint &ptCursorPos)
{
...
void CUITextView::SetSelectionMargin(BOOL bSelMargin)
{
...
void CUITextView::GetFont(LOGFONT &lf)
{
	lf = m_lfBaseFont;
}

void CUITextView::SetFont(const LOGFONT &lf)
{
	m_lfBaseFont = lf;
	m_nScreenLines = -1;
	m_nScreenChars = -1;
...
void CUITextView::OnUpdateIndicatorPosition(CCmdUI* pCmdUI)
{
	ASSERT_VALIDTEXTPOS(m_ptCursorPos);
	CString stat;
...
void CUITextView::OnUpdateIndicatorCRLF(CCmdUI* pCmdUI)
{
	if (m_pTextBuffer != NULL)
	{
...
void CUITextView::OnToggleBookmark(UINT nCmdID)
{
...
void CUITextView::OnGoBookmark(UINT nCmdID)
{
...
void CUITextView::OnClearBookmarks()
{
	if (m_pTextBuffer != NULL)
	{
...
void CUITextView::ShowCursor()
{
	m_bCursorHidden = FALSE;
	UpdateCaret();
}

void CUITextView::HideCursor()
{
	m_bCursorHidden = TRUE;
	UpdateCaret();
}

DROPEFFECT CUITextView::GetDropEffect()
{
	return DROPEFFECT_COPY;
}

void CUITextView::OnDropSource(DROPEFFECT de)
{
	ASSERT(de == DROPEFFECT_COPY);
}

BOOL CUITextView::IsDraggingText() const
{
	return m_bDraggingText;
}

HGLOBAL CUITextView::PrepareDragData()
{
	PrepareSelBounds();
	...
	HGLOBAL hData = ::GlobalAlloc(GMEM_MOVEABLE | GMEM_DDESHARE, text.GetLength() * sizeof(TCHAR) + 1);
	if (hData == NULL)
		return NULL;

	LPSTR pszData = (LPSTR) ::GlobalLock(hData);
	USES_CONVERSION;
	strcpy(pszData, T2A(text.GetBuffer()));
	text.ReleaseBuffer();
	::GlobalUnlock(hData);
...
static int FindStringHelper(LPCTSTR pszFindWhere, LPCTSTR pszFindWhat, BOOL bWholeWord)
{
	...
	for (;;)
	{
		...
		if (_istalnum(pszPos[nLength]) || pszPos[nLength] == _T('_'))
		{
...
BOOL CUITextView::HighlightText(const CPoint &ptStartPos, int nLength)
{
...
BOOL CUITextView::FindText(LPCTSTR pszText, const CPoint &ptStartPos, DWORD dwFlags, 
...
BOOL CUITextView::FindTextInBlock(LPCTSTR pszText, const CPoint &ptStartPosition, 
...
void CUITextView::OnEditRepeat() 
{
	if (m_bLastSearch)
	{
		...
		HighlightText(ptFoundPos, _tcslen(m_pszLastFindWhat));
		m_bMultipleSearch = TRUE;       // More search       
	}
}

void CUITextView::OnUpdateEditRepeat(CCmdUI* pCmdUI) 
{
	pCmdUI->Enable(m_bLastSearch);
}

void CUITextView::OnEditFindPrevious() 
{
...
void CUITextView::OnUpdateEditFindPrevious(CCmdUI* pCmdUI) 
{
	pCmdUI->Enable(m_bLastSearch);
}

void CUITextView::OnFilePageSetup()
{
	CWinApp *pApp = AfxGetApp();
	ASSERT(pApp != NULL);
...
void CUITextView::OnToggleBookmark()
{
	if (m_pTextBuffer != NULL)
	{
		DWORD dwFlags = GetLineFlags(m_ptCursorPos.y);
...
void CUITextView::OnNextBookmark()
{
	if (m_pTextBuffer != NULL)
	{
...
void CUITextView::OnPrevBookmark()
{
	if (m_pTextBuffer != NULL)
	{
...
void CUITextView::OnClearAllBookmarks()
{
	if (m_pTextBuffer != NULL)
	{
		int nLineCount = GetLineCount();
...
void CUITextView::OnUpdateNextBookmark(CCmdUI* pCmdUI)
{
	pCmdUI->Enable(m_bBookmarkExist);
}

void CUITextView::OnUpdatePrevBookmark(CCmdUI* pCmdUI)
{
	pCmdUI->Enable(m_bBookmarkExist);
}

void CUITextView::OnUpdateClearAllBookmarks(CCmdUI* pCmdUI)
{
	pCmdUI->Enable(m_bBookmarkExist);
}

BOOL CUITextView::GetViewTabs()
{
	return m_bViewTabs;
}

void CUITextView::SetViewTabs(BOOL bViewTabs)
{
...
BOOL CUITextView::GetSelectionMargin()
{
	return m_bSelMargin;
}

int CUITextView::GetMarginWidth()
{
	return m_bSelMargin ? 60 : 1;
}

int CUITextView::GetMarginMarkWidth()
{
	return m_bSelMargin ? 20 : 1;
}

BOOL CUITextView::GetSmoothScroll() const
{
	return m_bSmoothScroll;
}

void CUITextView::SetSmoothScroll(BOOL bSmoothScroll)
{
	m_bSmoothScroll = bSmoothScroll;
}

//	[JRT]
BOOL CUITextView::GetDisableDragAndDrop() const
{
	return m_bDisableDragAndDrop;
}

//	[JRT]
void CUITextView::SetDisableDragAndDrop(BOOL bDDAD)
{
	m_bDisableDragAndDrop = bDDAD;
}

void CUITextView::MoveLeft(BOOL bSelect)
{
	PrepareSelBounds();
...
void CUITextView::MoveRight(BOOL bSelect)
{
	PrepareSelBounds();
	if (m_ptDrawSelStart != m_ptDrawSelEnd && ! bSelect)
	{
...
void CUITextView::MoveWordLeft(BOOL bSelect)
{
	PrepareSelBounds();
	if (m_ptDrawSelStart != m_ptDrawSelEnd && ! bSelect)
	{
	...
	{
		nPos --;
		if (_istalnum(pszChars[nPos]) || pszChars[nPos] == _T('_'))
		{
			while (nPos > 0 && (_istalnum(pszChars[nPos - 1]) || pszChars[nPos - 1] == _T('_')))
				nPos --;
		}
		else
		{
			while (nPos > 0 && ! _istalnum(pszChars[nPos - 1])
				&& pszChars[nPos - 1] != _T('_') && ! _istspace(pszChars[nPos - 1]))
				nPos --;
		}
	}
...
void CUITextView::MoveWordRight(BOOL bSelect)
{
	PrepareSelBounds();
	if (m_ptDrawSelStart != m_ptDrawSelEnd && ! bSelect)
	{
	...
	if (_istalnum(pszChars[nPos]) || pszChars[nPos] == _T('_'))
	{
		while (nPos < nLength && _istalnum(pszChars[nPos]) || pszChars[nPos] == _T('_'))
			nPos ++;
	}
	else
	{
		while (nPos < nLength && ! _istalnum(pszChars[nPos])
			&& pszChars[nPos] != _T('_') && ! _istspace(pszChars[nPos]))
			nPos ++;
	}
...
void CUITextView::MoveUp(BOOL bSelect)
{
	PrepareSelBounds();
	if (m_ptDrawSelStart != m_ptDrawSelEnd && ! bSelect)
		m_ptCursorPos = m_ptDrawSelStart;
...
void CUITextView::MoveDown(BOOL bSelect)
{
	PrepareSelBounds();
	if (m_ptDrawSelStart != m_ptDrawSelEnd && ! bSelect)
		m_ptCursorPos = m_ptDrawSelEnd;
...
void CUITextView::MoveHome(BOOL bSelect)
{
	int nLength = GetLineLength(m_ptCursorPos.y);
	LPCTSTR pszChars = GetLineChars(m_ptCursorPos.y);
	int nHomePos = 0;
...
void CUITextView::MoveEnd(BOOL bSelect)
{
	m_ptCursorPos.x = GetLineLength(m_ptCursorPos.y);
	m_nIdealCharPos = CalculateActualOffset(m_ptCursorPos.y, m_ptCursorPos.x);
	EnsureVisible(m_ptCursorPos);
...
void CUITextView::MovePgUp(BOOL bSelect)
{
...
void CUITextView::MovePgDn(BOOL bSelect)
{
	int nNewTopLine = m_nTopLine + GetScreenLines() - 1;
...
void CUITextView::MoveCtrlHome(BOOL bSelect)
{
	m_ptCursorPos.x = 0;
	m_ptCursorPos.y = 0;
	m_nIdealCharPos = CalculateActualOffset(m_ptCursorPos.y, m_ptCursorPos.x);
...
void CUITextView::MoveCtrlEnd(BOOL bSelect)
{
	m_ptCursorPos.y = GetLineCount() - 1;
	m_ptCursorPos.x = GetLineLength(m_ptCursorPos.y);
	m_nIdealCharPos = CalculateActualOffset(m_ptCursorPos.y, m_ptCursorPos.x);
...
CPoint CUITextView::WordToRight(CPoint pt)
{
	ASSERT_VALIDTEXTPOS(pt);
	...
	while (pt.x < nLength)
	{
		if (! _istalnum(pszChars[pt.x]) && pszChars[pt.x] != _T('_'))
			break;
		pt.x ++;
	}
	ASSERT_VALIDTEXTPOS(pt);
...
CPoint CUITextView::WordToLeft(CPoint pt)
{
	ASSERT_VALIDTEXTPOS(pt);
	LPCTSTR pszChars = GetLineChars(pt.y);
	while (pt.x > 0)
	{
		if (! _istalnum(pszChars[pt.x - 1]) && pszChars[pt.x - 1] != _T('_'))
			break;
		pt.x --;
	}
	ASSERT_VALIDTEXTPOS(pt);
...
void CUITextView::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if (point.x < GetMarginWidth())
	{
		if (point.x > GetMarginMarkWidth())
		{
			...
			else
			{
				...
				SetCapture();
				m_nDragSelTimer = SetTimer(UI_TIMER_DRAGSEL, 100, NULL);
...
void CUITextView::OnMouseMove(UINT nFlags, CPoint point) 
{
...
void CUITextView::OnLButtonUp(UINT nFlags, CPoint point) 
{
	...
	else
	{
		...
		{
			AdjustTextPoint(point);
			CPoint ptOldPos = m_ptCursorPos;
			m_ptCursorPos = ClientToText(point); // 行号
			OnToggleBookmark();
			m_ptCursorPos = ptOldPos;
		}
	}
...
void CUITextView::OnTimer(UINT nIDEvent) 
{
	CView::OnTimer(nIDEvent);

	if (nIDEvent == UI_TIMER_DRAGSEL)
	{
...
void CUITextView::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
...
void CUITextView::OnEditCopy() 
{
	Copy();
}

void CUITextView::OnUpdateEditCopy(CCmdUI* pCmdUI) 
{
	pCmdUI->Enable(m_ptSelStart != m_ptSelEnd);
}

void CUITextView::OnEditSelectAll() 
{
	SelectAll();
}

void CUITextView::OnUpdateEditSelectAll(CCmdUI* pCmdUI) 
{
	pCmdUI->Enable(TRUE);
}

void CUITextView::OnRButtonDown(UINT nFlags, CPoint point) 
{
	CPoint pt = point;
	AdjustTextPoint(pt);
	pt = ClientToText(pt);
...
BOOL CUITextView::IsSelection()
{
	return m_ptSelStart != m_ptSelEnd;
}

void CUITextView::Copy()
{
	if (m_ptSelStart == m_ptSelEnd)
		return;
...
BOOL CUITextView::TextInClipboard()
{
	return IsClipboardFormatAvailable(CF_TEXT);
}

BOOL CUITextView::PutToClipboard(LPCTSTR pszText)
{
...
BOOL CUITextView::GetFromClipboard(CString &text)
{
	BOOL bSuccess = FALSE;
	if (OpenClipboard())
	{
