...
IMPLEMENT_DYNCREATE(CUIEditView, CUITextView)

CUIEditView::CUIEditView()
{
	AFX_ZERO_INIT_OBJECT(CUITextView);
	m_bAutoIndent = TRUE;
}
...
BEGIN_MESSAGE_MAP(CUIEditView, CUITextView)
	ON_WM_CREATE()
	ON_WM_DESTROY()
	ON_WM_CHAR()
	ON_WM_MOUSEWHEEL()
...
CEditDropTargetImpl::CEditDropTargetImpl( CUIEditView *pOwner )
{
	m_pOwner = pOwner;
}
...
BOOL CUIEditView::GetOverwriteMode() const
{
	return m_bOvrMode;
}

void CUIEditView::SetOverwriteMode(BOOL bOvrMode /*= TRUE*/)
{
	m_bOvrMode = bOvrMode;
}

BOOL CUIEditView::GetDisableBSAtSOL() const
{
	return m_bDisableBSAtSOL;
}

BOOL CUIEditView::GetAutoIndent() const
{
	return m_bAutoIndent;
}

void CUIEditView::SetAutoIndent(BOOL bAutoIndent)
{
	m_bAutoIndent = bAutoIndent;
}

void CUIEditView::ResetView()
{
	m_bAutoIndent = TRUE;
	m_bOvrMode = FALSE;
	CUITextView::ResetView();
}

BOOL CUIEditView::QueryEditable()
{
...
void CUIEditView::OnEditPaste() 
{
	Paste();
}

void CUIEditView::OnUpdateEditPaste(CCmdUI* pCmdUI) 
{
	pCmdUI->Enable(TextInClipboard());
}

void CUIEditView::OnUpdateEditCut(CCmdUI* pCmdUI) 
{
	pCmdUI->Enable(IsSelection());
}

void CUIEditView::OnEditCut() 
{
	Cut();
}

BOOL CUIEditView::DeleteCurrentSelection()
{
	if (IsSelection())
	{
...
void CUIEditView::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	CUITextView::OnChar(nChar, nRepCnt, nFlags);
...
//
void CUIEditView::OnEditDeleteBack() 
{
	if (IsSelection())
	{
		OnEditDelete();
...
void CUIEditView::OnUpdateIndicatorCol(CCmdUI* pCmdUI)
{
	pCmdUI->Enable(FALSE);
}

void CUIEditView::OnUpdateIndicatorOvr(CCmdUI* pCmdUI)
{
	pCmdUI->Enable(m_bOvrMode);
}

void CUIEditView::OnUpdateIndicatorRead(CCmdUI* pCmdUI)
{
	if (m_pTextBuffer == NULL)
		pCmdUI->Enable(FALSE);
	else
...
void CUIEditView::OnEditSwitchOvrmode() 
{
	m_bOvrMode = ! m_bOvrMode;
}

void CUIEditView::OnUpdateEditSwitchOvrmode(CCmdUI* pCmdUI) 
{
	pCmdUI->SetCheck(m_bOvrMode ? 1 : 0);
}

void CUIEditView::DoDragScroll(const CPoint &point)
{
	CRect rcClientRect;
...
BOOL CUIEditView::DoDropText(COleDataObject *pDataObject, const CPoint &ptClient)
{
...
int CUIEditView::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	if (CUITextView::OnCreate(lpCreateStruct) == -1)
		return -1;
	...
	{
		TRACE("Warning: Unable to register drop target for CUIEditView.\n");
...
void CUIEditView::OnDestroy() 
{
	...
	CUITextView::OnDestroy();
}

void CUIEditView::ShowDropIndicator(const CPoint &point)
{
...
void CUIEditView::HideDropIndicator()
{
	if (m_bDropPosVisible)
	{
...
DROPEFFECT CUIEditView::GetDropEffect()
{
	return DROPEFFECT_COPY | DROPEFFECT_MOVE;
}

void CUIEditView::OnDropSource(DROPEFFECT de)
{
	if (! IsDraggingText())
		return;
...
void CUIEditView::UpdateView(CUITextView *pSource, IUpdateContext *pContext, DWORD dwFlags, int nLineIndex /*= -1*/)
{
	CUITextView::UpdateView(pSource, pContext, dwFlags, nLineIndex);
...
void CUIEditView::OnEditReplace() 
{
	if (! QueryEditable())
		return;
...
BOOL CUIEditView::ReplaceSelection(LPCTSTR pszNewText)
{
...
void CUIEditView::OnUpdateEditUndo(CCmdUI* pCmdUI) 
{
...
//	[JRT]
void CUIEditView::SetDisableBSAtSOL(BOOL bDisableBSAtSOL)
{
	m_bDisableBSAtSOL = bDisableBSAtSOL;
}

void CUIEditView::OnEditRedo() 
{
...
void CUIEditView::OnUpdateEditRedo(CCmdUI* pCmdUI) 
{
...
void CUIEditView::OnEditOperation(int nAction, LPCTSTR pszText)
{
	if (m_bAutoIndent)
	{
		//	Analyse last action...
...
BOOL CUIEditView::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt) 
{
	if( zDelta != 0 ) {
	...
	return CUITextView::OnMouseWheel(nFlags, zDelta, pt);
}
