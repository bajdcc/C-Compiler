...
/////////////////////////////////////////////////////////////////////////////
// CUITextBuffer::SUndoRecord

void CUITextBuffer::SUndoRecord::SetText(LPCTSTR pszText)
{
    m_pszText = NULL;
...
void CUITextBuffer::SUndoRecord::FreeText()
{
...
/////////////////////////////////////////////////////////////////////////////
// CUITextBuffer::CUpdateContext

void CUITextBuffer::CInsertContext::RecalcPoint(CPoint &ptPoint)
{
...
void CUITextBuffer::CDeleteContext::RecalcPoint(CPoint &ptPoint)
{
    ASSERT(m_ptEnd.y > m_ptStart.y ||
        m_ptEnd.y == m_ptStart.y && m_ptEnd.x >= m_ptStart.x);
    if (ptPoint.y < m_ptStart.y)
...
IMPLEMENT_DYNAMIC(CUITextBuffer, CCmdTarget)

CUITextBuffer::CUITextBuffer()
{
    m_bInit = FALSE;
    m_bReadOnly = FALSE;
    m_bModified = FALSE;
...
BEGIN_MESSAGE_MAP(CUITextBuffer, CCmdTarget)
END_MESSAGE_MAP()


// CUITextBuffer 消息处理程序

void CUITextBuffer::InsertLine(LPCTSTR pszLine, int nLength /*= -1*/, int nPosition /*= -1*/)
{
	if (nLength == -1)
	{
		if (pszLine == NULL)
...
void CUITextBuffer::AppendLine(int nLineIndex, LPCTSTR pszChars, int nLength /*= -1*/)
{
	if (nLength == -1)
	{
		if (pszChars == NULL)
...
BOOL CUITextBuffer::InitNew(int nCrlfStyle /*= CRLF_STYLE_DOS*/)
{
	ASSERT(! m_bInit);
...
BOOL CUITextBuffer::GetReadOnly() const
{
...
void CUITextBuffer::SetReadOnly(BOOL bReadOnly /*= TRUE*/)
{
	ASSERT(m_bInit);	//	Text buffer not yet initialized.
	//	You must call InitNew() or LoadFromFile() first!
...
BOOL CUITextBuffer::LoadFromFile(LPCTSTR pszFileName, int nCrlfStyle /*= CRLF_STYLE_AUTOMATIC*/)
{
	ASSERT(! m_bInit);
	ASSERT(m_aLines.GetSize() == 0);
	...
	__try
	{
		...
		if (hFile == INVALID_HANDLE_VALUE)
			__leave;
		if (::GetFileSize(hFile, NULL) > 0x00050000) // Too large
			__leave;
...
BOOL CUITextBuffer::SaveToFile(LPCTSTR pszFileName, int nCrlfStyle /*= CRLF_STYLE_AUTOMATIC*/, BOOL bClearModifiedFlag /*= TRUE*/)
{
	...
	__try
	{
		...
		{
			TCHAR* pstrLine = m_aLines[nLine].m_pcLine;
			int nLength = m_aLines[nLine].m_nLength;
			DWORD dwWrittenBytes;
			if (nLength > 0)
			{
				int nLen = nLength;
				for (int i = 0; i < nLength; i++, pstrLine++)
					if (*pstrLine < _T('\0') || *pstrLine > _T('\xFF'))
						nLen++;
				if (! ::WriteFile(hTempFile, T2A(m_aLines[nLine].m_pcLine), nLen, &dwWrittenBytes, NULL))
					__leave;
				if (nLen != (int) dwWrittenBytes)
					__leave;
			}
...
int CUITextBuffer::GetCRLFMode()
{
	return m_nCRLFMode;
}

void CUITextBuffer::SetCRLFMode(int nCRLFMode)
{
...
int CUITextBuffer::GetLineCount()
{
	ASSERT(m_bInit);	//	Text buffer not yet initialized.
	//	You must call InitNew() or LoadFromFile() first!
...
int CUITextBuffer::GetLineLength(int nLine)
{
	ASSERT(m_bInit);	//	Text buffer not yet initialized.
	//	You must call InitNew() or LoadFromFile() first!
...
LPTSTR CUITextBuffer::GetLineChars(int nLine)
{
	ASSERT(m_bInit);	//	Text buffer not yet initialized.
	//	You must call InitNew() or LoadFromFile() first!
...
DWORD CUITextBuffer::GetLineFlags(int nLine)
{
	ASSERT(m_bInit);	//	Text buffer not yet initialized.
	//	You must call InitNew() or LoadFromFile() first!
...
int CUITextBuffer::FindLineWithFlag(DWORD dwFlag)
{
...
int CUITextBuffer::GetLineWithFlag(DWORD dwFlag)
{
...
void CUITextBuffer::SetLineFlag(int nLine, DWORD dwFlag, BOOL bSet, BOOL bRemoveFromPreviousLine /*= TRUE*/)
{
	ASSERT(m_bInit);	//	Text buffer not yet initialized.
	//	You must call InitNew() or LoadFromFile() first!
...
void CUITextBuffer::GetText(int nStartLine, int nStartChar, int nEndLine, int nEndChar, CString &text, LPCTSTR pszCRLF /*= NULL*/)
{
	ASSERT(m_bInit);	//	Text buffer not yet initialized.
	//	You must call InitNew() or LoadFromFile() first!
...
void CUITextBuffer::AddView(CUITextView *pView)
{
	m_lpViews.AddTail(pView);
}

void CUITextBuffer::RemoveView(CUITextView *pView)
{
	POSITION pos = m_lpViews.GetHeadPosition();
	while (pos != NULL)
	{
		POSITION thispos = pos;
		CUITextView *pvw = m_lpViews.GetNext(pos);
		if (pvw == pView)
		{
...
void CUITextBuffer::UpdateViews(CUITextView *pSource, IUpdateContext *pContext, DWORD dwUpdateFlags, int nLineIndex /*= -1*/)
{
	POSITION pos = m_lpViews.GetHeadPosition();
	while (pos != NULL)
	{
		CUITextView *pView = m_lpViews.GetNext(pos);
		pView->UpdateView(pSource, pContext, dwUpdateFlags, nLineIndex);
	}
}

BOOL CUITextBuffer::InternalDeleteText(CUITextView *pSource, int nStartLine, int nStartChar, int nEndLine, int nEndChar)
{
	ASSERT(m_bInit);	//	Text buffer not yet initialized.
	//	You must call InitNew() or LoadFromFile() first!
	ASSERT(nStartLine >= 0 && nStartLine < m_aLines.GetSize());
...
BOOL CUITextBuffer::InternalInsertText(CUITextView *pSource, int nLine, int nPos, LPCTSTR pszText, int &nEndLine, int &nEndChar)
{
	ASSERT(m_bInit);	//	Text buffer not yet initialized.
	//	You must call InitNew() or LoadFromFile() first!
	...
	for (;;)
	{
		...
		if (nTextPos > MAX_LINE_CHARS) // 一行太多字符
		{
			return FALSE;
		}
...
BOOL CUITextBuffer::CanUndo()
{
	ASSERT(m_nUndoPosition >= 0 && m_nUndoPosition <= m_aUndoBuf.GetSize());
	return m_nUndoPosition > 0;
}

BOOL CUITextBuffer::CanRedo()
{
	ASSERT(m_nUndoPosition >= 0 && m_nUndoPosition <= m_aUndoBuf.GetSize());
	return m_nUndoPosition < m_aUndoBuf.GetSize();
}

POSITION CUITextBuffer::GetUndoDescription(CString &desc, POSITION pos /*= NULL*/)
{
...
POSITION CUITextBuffer::GetRedoDescription(CString &desc, POSITION pos /*= NULL*/)
{
...
BOOL CUITextBuffer::Undo(CPoint &ptCursorPos)
{
	ASSERT(CanUndo());
	ASSERT((m_aUndoBuf[0].m_dwFlags & UNDO_BEGINGROUP) != 0);
	for (;;)
	{
		...
		if (ur.m_dwFlags & UNDO_INSERT)
		{
			...
			ASSERT(_tcscmp(text, ur.GetText()) == 0);
#endif
...
BOOL CUITextBuffer::Redo(CPoint &ptCursorPos)
{
	ASSERT(CanRedo());
	ASSERT((m_aUndoBuf[0].m_dwFlags & UNDO_BEGINGROUP) != 0);
	ASSERT((m_aUndoBuf[m_nUndoPosition].m_dwFlags & UNDO_BEGINGROUP) != 0);
...
void CUITextBuffer::AddUndoRecord(BOOL bInsert, const CPoint &ptStartPos, const CPoint &ptEndPos, LPCTSTR pszText, int nActionType)
{
...
BOOL CUITextBuffer::InsertText(CUITextView *pSource, int nLine, int nPos, LPCTSTR pszText, 
...
BOOL CUITextBuffer::DeleteText(CUITextView *pSource, int nStartLine, int nStartChar, 
	int nEndLine, int nEndChar, int nAction)
{
...
BOOL CUITextBuffer::GetActionDescription(int nAction, CString &desc)
{
	HINSTANCE hOldResHandle = AfxGetResourceHandle();
	if (CUITextView::s_hResourceInst != NULL)
		AfxSetResourceHandle(CUITextView::s_hResourceInst);
	BOOL bSuccess = FALSE;
	switch (nAction)
	{
...
void CUITextBuffer::SetModified(BOOL bModified /*= TRUE*/)
{
	m_bModified = bModified;
}

BOOL CUITextBuffer::IsModified() const
{
	return m_bModified;
}

void CUITextBuffer::BeginUndoGroup(BOOL bMergeWithPrevious /*= FALSE*/)
{
	ASSERT(! m_bUndoGroup);
	m_bUndoGroup = TRUE;
...
void CUITextBuffer::FlushUndoGroup(CUITextView *pSource)
{
	ASSERT(m_bUndoGroup);
	if (pSource != NULL)
	{
...
int CUITextBuffer::FindNextBookmarkLine(int nCurrentLine)
{
	BOOL bWrapIt = TRUE;
...
int CUITextBuffer::FindPrevBookmarkLine(int nCurrentLine)
{
	BOOL bWrapIt = TRUE;
	DWORD dwFlags = GetLineFlags(nCurrentLine);
	if ((dwFlags & LF_BOOKMARKS) != 0)
